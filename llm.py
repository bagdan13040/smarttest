...
def generate_open_questions(topic, n=5, difficulty='средний', api_key=None):
    ...
    prompt = (
        ...
        "Вопросы должны быть понятными и доступными, фокусируйся на ключевых концепциях темы, избегая излишней глубины. "
...
def evaluate_answer(question_text, user_answer, notes="", api_key=None):
    ...
    data = {
        "model": "xiaomi/mimo-v2-flash:free",
        "messages": [
            {"role": "system", "content": "Ты — поддерживающий и вовлекающий преподаватель. Оценивай лояльно, старайся найти правильные мысли даже в неполных ответах. Верни только JSON."},
            {"role": "user", "content": prompt}
        ]
    }
