...
def generate_learning_roadmap(topic, goal=None, level="начинающий", api_key=None, interests=None):
    ...
    if interests:
        interests_text = (
            f"\n\nКРИТИЧЕСКИ ВАЖНО: Студент очень увлекается темой '{interests}'. "
            f"ОБЯЗАТЕЛЬНО адаптируй названия и содержание модулей так, чтобы они использовали "
            f"аналогии или примеры из сферы '{interests}'. Сделай обучение максимально персонализированным."
        )
    else:
        interests_text = ""
...
Никаких описаний, только списки."""
    ...
    data = {
        ...
        "messages": [
            {
                "role": "system", 
                "content": "Ты — API, возвращающий только сырой JSON. Твоя задача — создать персонализированный курс, используя интересы студента для повышения вовлеченности."
            },
...
def chat_with_image(message, image_path=None, history=None, api_key=None, model="arcee-ai/trinity-large-preview:free"):
    """
...
def generate_quiz(topic, difficulty="средний", api_key=None, interests=None):
    ...
    if interests:
        interests_context = (
            f"\n\nКРИТИЧЕСКИ ВАЖНО: Студент очень увлекается темой '{interests}'. "
            f"Ты ОБЯЗАН адаптировать теорию под эти интересы. Используй примеры, аналогии и ассоциации, "
            f"связанные с '{interests}', чтобы объяснить сложные концепции. "
            f"Например, если тема 'Аэродинамика' и интерес 'Гонки', объясняй прижимную силу на примере болидов F1. "
            f"Это должно быть органично, но заметно."
        )
    else:
        interests_context = ""
    ...
    prompt = (
        ...
        "\n\nИНСТРУКЦИИ:\n"
        "1. Напиши теоретический материал объемом не менее 1000 слов, с глубиной и детализацией. "
        "Раздели его на логические разделы и подразделы, поясняй термины, приводи примеры. "
        "Используй теги [b]...[/b] для важных понятий. "
        "НЕ используй тэги [p], [h1] и т.д. Просто используй переносы строк для абзацев. "
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "Ты — экспертный преподаватель и API. Твоя задача — генерировать образовательный контент в формате JSON. "
                    "Используй ТОЛЬКО разрешенные Kivy markup теги: [b], [i], [color=...], [size=...]. "
                    "ЗАПРЕЩЕНО использовать теги [p], [h1], [h2], [br]. Для разделения абзацев используй двойной перенос строки \\n\\n. "
                    "Маркдаун (** или #) строго запрещен. "
                    "Если указаны интересы студента, обязательно интегрируй их в теорию через примеры."
                )
            },
            {
                "role": "user",
...
def generate_open_questions(topic, n=5, difficulty='средний', api_key=None):
    ...
    prompt = (
        ...
        f"Сгенерируй {n} открытых вопросов по теме. "
        "Вопросы должны быть простыми и понятными, чтобы на них можно было ответить кратко и по сути (1-2 предложения). "
        "Фокусируйся на самых базовых и важных концепциях. "
        "Для каждого вопроса укажи 'notes' — максимально краткий список ключевых фактов, которые должны быть в ответе. "
        "Верни ТОЛЬКО JSON массив объектов. "
        "Структура: [{\"id\": \"q1\", \"question\": \"Текст вопроса...\", \"notes\": \"Минимум ключевых фактов...\"}, ...]"
    )
...
def evaluate_answer(question_text, user_answer, notes="", api_key=None):
    ...
    prompt = (
        ...
        "1. Оцени ответ по шкале от 0 до 10.\n"
        "2. Если ответ технически правильный и по существу, ставь 9-10 баллов, даже если он очень короткий.\n"
        "3. НЕ снижай балл за отсутствие 'воды' или краткость. Цени точность и лаконичность.\n"
        "4. Если в ответе есть правильная мысль, но он неполный, будь лоялен и ставь 7-8 баллов.\n"
        "5. Дай доброжелательный комментарий, список ошибок (если есть) и предложения по улучшению.\n"
        "Верни ТОЛЬКО JSON объект.\n"
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "Ты — лояльный и поддерживающий преподаватель. "
                    "Твоя задача — поощрять знания, а не многословие. "
                    "Цени технически верные и лаконичные ответы. "
                    "Если студент ответил кратко, но правильно — это 10 баллов. "
                    "Не требуй 'воды' и длинных вступлений. "
                    "Верни только JSON."
                )
            },
            {"role": "user", "content": prompt}
